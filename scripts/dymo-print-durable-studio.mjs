/**
 * LW Durable 1933085 — studio face print test (rev 28).
 * node scripts/dymo-print-durable-studio.mjs
 */
...
function mapQr(el) {
  const rect = clamp(canvasBounds(el, durable), face(durable))
...
async function main() {
  const textB = clamp(canvasBounds(TEXT, durable), face(durable))
  const qrB = mapQr(QR)
  const lines = ["1' Cat6 Patch Cable", 'SKU:CIS326001']
  console.log('rev28 durable text', textB)
  console.log('rev28 durable qr', qrB)
  await printXml('DURABLE-rev28-studio', dieCut(textXml(lines, textB, 18) + (await qrImageXml(qrB))))
}
