...
function createBarcodeDetector(): BarcodeDetector | null {
  ...
  try {
    return new BarcodeDetector({ formats: DETECTOR_FORMATS })
  } catch {
    try {
      return new BarcodeDetector({ formats: ['qr_code', 'ean_13', 'upc_a', 'code_128'] })
    } catch {
      return null
    }
  }
...
function startPerFrameVideoDetect(opts: {
...
export default function BarcodeScanner({ onScan, onClose }: BarcodeScannerProps) {
  ...
  useEffect(() => {
    ...
    return startPerFrameVideoDetect({
    ...
    <div className="qr-scanner-overlay">
      <div className="qr-scanner-header">
        <div className="qr-scanner-header-text">
          <h3>Scan barcode</h3>
          <p className="qr-scanner-hint">Point the camera at a UPC, QR, or other product barcode.</p>
        </div>
